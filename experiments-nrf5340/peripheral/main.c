...
// #define SECRET_ACCESS // uncomment this to verify user mode cannot read secret
static uint8_t secret[7] = {'s','e','c','r','e', 't',0};

void attack_worked(){
	printk("The attack worked!\n");
}

void benign_func(){
	printk("This is a benign function!\n");
}

static void* func_ptr = &benign_func;
...
void user_access_secret(){
	printk("Hello World from %s (%s)\n",
        k_is_user_context() ? "UserSpace!" : "privileged mode.",
        CONFIG_BOARD_TARGET);
	printk("Trying to access secret in user mode...\n");
...
int main(void)
{
	 ...
	 */
	#ifdef SECRET_ACCESS
	k_thread_create(&user_thread, user_stack, USER_STACKSIZE, user_access_secret, NULL, NULL, NULL, -1, K_USER, K_MSEC(0));
	#endif

	printk("Secret data is at %p\n", secret);
	printk("Benign function is at %p\n", benign_func);
	printk("Evil function is at %p\n", attack_worked);
	printk("Func pointer is at %p\n", &func_ptr);
	void (*fptr)() = (void (*)())func_ptr;
	printk("Executing function pointer!\n");
	fptr();
